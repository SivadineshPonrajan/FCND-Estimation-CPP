...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	float phi_dot = gyro.x + sin(rollEst) * tan(pitchEst) * gyro.y + cos(rollEst) * tan(pitchEst) * gyro.z;
	float theta_dot = cos(rollEst) * gyro.y - sin(rollEst) * gyro.z;
	float psi_dot = sin(rollEst) * (1 / cos(pitchEst)) * gyro.y + cos(rollEst) * (1 / cos(pitchEst)) * gyro.z;
	float predictedRoll = rollEst + phi_dot * dtIMU;
	float predictedPitch = pitchEst + theta_dot * dtIMU;
	ekfState(6) = ekfState(6) + dtIMU * psi_dot;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	V3F main = attitude.Rotate_BtoI(accel);
	predictedState(0) += curState(3) * dt;
	predictedState(1) += curState(4) * dt;
	predictedState(2) += curState(5) * dt;
	predictedState(3) += main.x * dt;
	predictedState(4) += main.y * dt;
	predictedState(5) += main.z * dt - CONST_GRAVITY * dt;;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
	...
	hPrime(0, 6) = 1;
	zFromX(0) = ekfState(6);
	float yaw_estimation_error = magYaw - ekfState(6);

	if (yaw_estimation_error > F_PI) {
		zFromX(0) += 2.f*F_PI;
	}
	else if (yaw_estimation_error < -F_PI) {
		zFromX(0) -= 2.f*F_PI;
	}
